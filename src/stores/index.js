...
export const useProductStore = defineStore('products', {
  ...
  actions: {
      ...
      try {
        ...
        setTimeout(() => {
          this.products = [
            {
              id: 1,
              name: 'Гвинт самонарізний 5.8x70',
              price: 2.5,
              image: '/images/screw.jpg',
              slug: 'gvint-samonariznyj-5-8x70',
              subcategory: 1,
              code: 'SCR-001',
              description: 'Якісний гвинт самонарізний для будівельних робіт',
            },
            {
              id: 2,
              name: 'Фарба інтерєрна 5л',
              price: 320,
              image: '/images/paint.jpg',
              slug: 'farba-intererna-5l',
              subcategory: 2,
              code: 'PNT-001',
              description: 'Фарба для внутрішніх робіт',
            },
            {
              id: 3,
              name: 'Набір інструментів 56шт',
              price: 1200,
              image: '/images/toolset.jpg',
              slug: 'nabir-instrumentiv-56sht',
              subcategory: 3,
              code: 'TLS-001',
              description: 'Повний набір інструментів для домашнього майстра',
            },
          ]
          this.loading = false
        }, 500)
    ...
    /**
     * Добавляет набор товаров в выбранную категорию
     * @param {Array} products - Массив товаров для добавления
     * @param {string} categorySlug - Слаг категории
     * @returns {Promise} - Промис завершения операции
     */
    async addBulkProducts(products, categorySlug) {
      try {
        ...
        // В реальном приложении здесь был бы API запрос для сохранения товаров

        // Для демо просто добавляем товары в локальное хранилище
        const newProducts = products.map((product) => {
          // Убедимся, что каждый товар привязан к категории
          return {
            ...product,
        ...
        // Сохраняем товары (в реальном приложении здесь был бы API-вызов)
        console.log(`Добавлено ${newProducts.length} товаров в категорию ${category.name}`)

        // Для демо, чтобы показать товары, добавим их в локальное хранилище
        // В реальной реализации товары загружались бы с сервера
        const existingProducts = JSON.parse(
          localStorage.getItem(`products_${categorySlug}`) || '[]',
        )
      ...
      return new Promise((resolve) => {
        setTimeout(() => {
          ...
          this.orders = [
            {
              id: '1.250523',
              items: [{ id: 1, name: 'Гвинт самонарізний 5.8x70', price: 2.5, quantity: 10 }],
              total: 25.0,
              status: 'DELIVERED',
            ...
            {
              id: '2.260523',
              items: [{ id: 2, name: 'Фарба інтерєрна 5л', price: 320, quantity: 1 }],
              total: 320.0,
              status: 'PAID',
